...
void ActionRecognitionProject::clusterMoSIFTPoints()
{
	...
	BagOfWordsRepresentation bow_rep(filenames, NUM_CLUSTERS, FEATURE_DIMENSIONALITY, NUMBER_OF_PEOPLE, false, false);
...
void ActionRecognitionProject::buildMoFREAKFromVideos()
{
	...
	for (auto it = files.begin(); it != files.end(); ++it)
	{
		...
		// and compute.
		mofreak.computeMoFREAKFromFile(it->toStdString(), true);
	}
...
void ActionRecognitionProject::loadMoFREAKFilesForClustering()
{
	...
	for (auto it = files.begin(); it != files.end(); ++it)
	{
		mofreak.readMoFREAKFeatures(it->toStdString());
	}
